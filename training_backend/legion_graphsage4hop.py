...
def train_one_step(model, optimizer, loss_fcn, device, feat_len, iter, device_id):
    ...
    ids, features, labels, block1_agg_src, block1_agg_dst, block2_agg_src, block2_agg_dst, block3_agg_src, block4_agg_dst, block4_agg_src, block3_agg_dst = ipc_service.get_next(feat_len)
    block1_src_num, block1_dst_num, block2_src_num, block2_dst_num, block3_src_num, block3_dst_num, block4_src_num, block4_dst_num = ipc_service.get_block_size()
    print(block1_src_num, block1_dst_num, block2_src_num, block2_dst_num, block3_src_num, block3_dst_num, block4_src_num, block4_dst_num)
    blocks = []
    ...
    blocks.append(create_dgl_block(block4_agg_src, block4_agg_dst, block4_src_num, block4_dst_num))
...
if __name__ == "__main__":
    ...
    argparser.add_argument('--hops_num', type=int, default=4)
    argparser.add_argument('--nbrs_num', type=list, default=[25, 10, 5, 2])
