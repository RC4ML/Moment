...
class PNA(nn.Module):
    ...
    def forward(self, blocks, x, labels):
        ...
        for l, (layer, block) in enumerate(zip(self.layers, blocks)):
            ...
            eid = torch.arange(start=0, end=bg.num_edges(), step=1).to('cuda').int()
            gret = dgl.graph(bg.find_edges(eid))
            h = h[:gret.batch_num_nodes()]
            h = layer(gret, h)
...
def create_dgl_block(src, dst, num_src_nodes, num_dst_nodes):
    ...
    block = dgl.create_block((src, dst), num_src_nodes, num_dst_nodes)
    # print(block)
    # print(g)
    # print(src.shape)
