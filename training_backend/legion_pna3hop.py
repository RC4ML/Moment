...
class PNA(nn.Module):
    def __init__(self,
        ...
        self.layers.append(PNAConv(in_feats, n_hidden, ['mean', 'max', 'sum'], ['identity', 'amplification'], 2.5))
        for _ in range(1, n_layers - 1):
            self.layers.append(PNAConv(n_hidden, n_hidden, ['mean', 'max', 'sum'], ['identity', 'amplification'], 2.5))
        self.layers.append(PNAConv(n_hidden, n_classes, ['mean', 'max', 'sum'], ['identity', 'amplification'], 2.5))
        self.dropout = nn.Dropout(dropout)
        self.activation = activation

    def forward(self, blocks, x, labels):
        h = x
        for l, (layer, block) in enumerate(zip(self.layers, blocks)):
            h = h[:block.batch_num_nodes()]
            h = layer(block, h)
...
def create_dgl_block(src, dst, num_src_nodes, num_dst_nodes):
    ...
    # g = dgl.create_block((src, dst), num_src_nodes, num_dst_nodes)
    # print(g)
    # print(src.shape)
    # g_homo = dgl.to_homogeneous(g)
    # print(g_homo)
    g_homo = dgl.graph((src, dst))
    return g_homo

def train_one_step(model, optimizer, loss_fcn, device, feat_len, iter, device_id):
    
    ids, features, labels, block1_agg_src, block1_agg_dst, block2_agg_src, block2_agg_dst, block3_agg_src, block3_agg_dst = ipc_service.get_next(feat_len)
    block1_src_num, block1_dst_num, block2_src_num, block2_dst_num, block3_src_num, block3_dst_num = ipc_service.get_block_size()
    ...
    blocks.append(create_dgl_block(block3_agg_src, block3_agg_dst, block3_src_num, block3_dst_num))

    # print(features[:100])
    # print(ids[:100])
    batch_pred = model(blocks, features, labels)
...
def worker_process(rank, world_size, args):
    ...
    model = PNA(in_feats=args.features_num,
...
if __name__ == "__main__":
    ...
    argparser.add_argument('--train_batch_size', type=int, default=1000)
    argparser.add_argument('--hidden_dim', type=int, default=256)
    argparser.add_argument('--hops_num', type=int, default=3)
    argparser.add_argument('--nbrs_num', type=list, default=[25, 10, 5])
    ...
    worker_process(0, world_size, args)
